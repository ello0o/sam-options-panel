...
/** 
* Sam Theme & Plugin Options
* Version 1.0.0
* Author: Themology
*
*
* @package Sam Theme & Plugin Options
* @version 1.0.0
* @author Themology
* 
*/
...
* 
* @since Sam Theme & Plugin Options 1.0.0
*
...
* @param string $prefix Options unique prefix. Default: 'sr'. 
...
if ( ! class_exists('SamOptions') ) {
	class SamOptions{
		...
		public function __construct( 
			...
			if ( $sidebar === true && SamSidebars::$_count < 1) {
				$sidebar = new SamSidebars( $this->_url, $this->_folder );
			}
		...
		function load_scripts() {
			if ( strpos( get_current_screen()->id, $this->_slug ) ) {
				...
				if ( $this->_style !== false ) {
					wp_enqueue_style( 'themo-google-font-roboto', 'http://fonts.googleapis.com/css?family=Roboto:400,300,700,900', true );
				}
				
				wp_enqueue_style('themo-sam-select', $this->_url . '/'.$this->_folder.'/css/selectric.css', true);
				wp_enqueue_style('themo-sam-chosen', $this->_url.'/'.$this->_folder.'/css/chosen.css', true);
				wp_enqueue_style('themo-saml-fontawesome','//netdna.bootstrapcdn.com/font-awesome/4.0.3/css/font-awesome.min.css', true);
				wp_enqueue_style('themo-sam-main', $this->_url.'/'.$this->_folder.'/css/main.css', true);
				...
				wp_enqueue_media();
				wp_enqueue_script('themo-sam-selectric', $this->_url.'/'.$this->_folder.'/js/jquery.selectric.min.js' , 'jquery', null, true);
				wp_enqueue_script('themo-sam-chosen', $this->_url.'/'.$this->_folder.'/js/jquery.chosen.min.js' , 'jquery', null, true);
				wp_enqueue_script('themo-sam-main', $this->_url.'/'.$this->_folder.'/js/main.js' , 'jquery', null, true);
				
				wp_localize_script('themo-sam-main', 'js_options_data', array(
					1 => $this->_url,
					2 => __('Saving', 'samr'),
					3 => __('You are about to leave this page without saving. All changes will be lost.', 'samr'),
					4 => __('WARNING: You are about to delete all your settings! Please confirm this action.', 'samr'),
					5 => $this->_prefix,
					6 => __('WARNING: You are about to restore your backup. This will overwrite all your settings! Please confirm this action.', 'samr'),
					7 => __('WARNING: You are about to delete your backup. All unsaved options will be lost. We recommend that you save your options before deleting a backup. Please confirm this action.', 'samr'),
					8 => __('WARNING: You are about to create a backup. All unsaved options will be lost. We recommend that you save your options before deleting a backup. Please confirm this action.', 'samr'),
					9 => __('Delete','samr'),
					10=> $this->_prefix
				));
				...
				wp_add_inline_style('themo-sam-main', $css);
			} 
		}
		...
		function create_page() {
			if ( $this->_options ) {
				...
				if ( isset( $tabs ) ) {
					...
					foreach ( $tabs as $tab ) {
						...
						if ( isset( ${'tab_'.$tab['id']} ) ) {
							foreach ( ${'tab_'.$tab['id']} as $tab_content ) {
								$option = new SamOptionsGenerator( $tab_content, $this->_prefix );
								$div_contents .= $option;
							}
						} else {
							$div_contents .= __('There are no options defined for this tab.', 'samr');
						}
					...
					foreach ( ${'tab_0'} as $tab_content ) {
						$option = new SamOptionsGenerator( $tab_content );
						$div_contents .= $option;
					}
			...
			$html = '<div id="theme-options-wrapper">';
				...
				$html .= '<a href="#" id="save-options-bottom" class="'.( ( isset($no_tab) && $no_tab === true ) ? 'no-tab' : null ).'" title="'.__('Save Options','samr').'">'.__('Save Options','samr').'</a>';
			$html .= '</div>';
			$html .= '<a href="#" id="save-options-top" title="'.__('Quick Save','samr').'">'.__('Quick Save','samr').'</a>';
			$html .= '<div id="options-saved"><div class="fa fa-save fa-large fa-5x"></div><strong>'.__('Saved','samr').'</strong></div>';
			$html .= '<div id="options-error"><div class="fa fa-warning fa-large fa-5x"></div><strong>'.__('Error','samr').'</strong></div>';
...
if ( ! class_exists( 'SamOptionsGenerator' ) ) {
	class SamOptionsGenerator {
	...
	public function __construct( $data = null, $prefix = 'samr' ) {
		$this->_data 			= $data;
		...
		$this->_upload_title 	= __('Insert ', 'samr') . $this->_name;
		$this->_upload_button	= __('Choose as ', 'samr') . $this->_name;
	...
	function image() {
		$output = '<div class="form-control '.$this->_class.'">';
			...
			$output .= '<a href="#" class="image-upload-button btn" data-upload-title="'.$this->_upload_title.'" data-upload-button="'.$this->_upload_button.'">'.__('Upload','samr').'</a>';
			$output .= '<a href="#" class="image-clear-button btn" style="display:'.( ( $this->_value ) ? 'inline-block' : 'none').'">'.__('Clear','samr').'</a>';
	...
	function backup_button() {
		$output = '<div class="form-control '.$this->_class.'">';
			...
			$output .= '<div class="message '.$css.'">';
				$output .= '<h3 class="even">'.__('Back-up available','samr').'</h3>';
				$output .= '<p class="even">'.__('You options have been backed up. You can always restore your options by clicking the <strong>Restore</strong> button below:','samr').'</p>';
				$output .= '<ul class="backup-list even">';
				foreach ( $current_list as $backup ) {
				
					$output .= '<li>'.date( "M d, Y H:i", $backup ).'<a href="#" class="delete-backup" data-backup="'.$backup.'">'.__('Delete','samr').'</a><a href="#" class="restore-backup" data-backup="'.$backup.'">'.__('Restore','samr').'</a></li>';
				}
				$output .= '</ul>';
				$output .= '<p class="odd">'.__('You currently have not backups. You can create a backup by clicking the <strong>Backup Now</strong> link below:','samr').'</p>'; 
				$output .= '<a href="#" id="backup">'.__('Backup Now','samr').'</a>';
			$output .= '</div>';
			$output .= $this->_desc;
		$output .= '</div>';
	...
	function reset_button() {
		$output = '<div class="form-control '.$this->_class.'">';
			$output .= '<label class="name" for="'.$this->_id.'">'.$this->_name.'</label>';
			$output .= $this->_desc;
			$output .= '<a href="#" id="reset-options-bottom" title="'.__('Reset Options','samr').'">'.__('Reset Options','samr').'</a>';
		$output .= '</div>';
	...
	function import() {
		$output = '<div class="form-control '.$this->_class.'">';
			...
			$output .= $this->_desc;
			$output .= '<a href="#" id="import-options" title="'.__('Import Options','samr').'">'.__('Import Options','samr').'</a>';
		$output .= '</div>';
...
if ( ! class_exists( 'SamSidebars' ) ) {
	...
	function load_scripts() {
		...
		if ( get_current_screen()->id == 'appearance_page_sidebars' ) {
			...
			wp_register_style('themo-google-font-roboto', 'http://fonts.googleapis.com/css?family=Roboto:400,300,700,900', true);
			wp_register_style('themo-sidebars', $this->_url.'/'.$this->_folder.'/css/sidebars.css', null,  null, null);
			wp_register_script('themo-sidebars', $this->_url.'/'.$this->_folder.'/js/sidebars.js', array('jquery'), null, true);
			
			if ( ! wp_script_is( 'themo-google-font-roboto', 'enqueued' ) ) {
				wp_enqueue_style( 'themo-google-font-roboto' );
			}
			
			if ( ! wp_script_is( 'themo-google-font-roboto', 'enqueued' ) ) {
				wp_enqueue_style( 'themo-sidebars' );
			}
			
			if ( ! wp_script_is( 'themo-google-font-roboto', 'enqueued' ) ) {
				wp_enqueue_script( 'themo-sidebars' );
			}
			
			$js_data = array(
				__('Remove','samr'),
				__('Are you sure you want to delete this sidebar?','samr'),
				__('Sidebar name cannot be empty. Please provide a valid name for your sidebar.','samr'),
				__('You already have a sidebar with that name. Please provide a valid name for your sidebar.','samr'),
				__('Your sidebar has been succesfully created.','samr'),
				__('You currently have no sidebars created. <br>Use the form above to create your first sidebar.','samr')
			);
			
			wp_localize_script('themo-sidebars', 'js_data', $js_data);
			...
			wp_add_inline_style('themo-sidebars', $color_scheme);
		} 
	}
	...
	function add_submenu_page(){
	     add_submenu_page( 'themes.php', __('Sidebars', 'samr'), __('Sidebars', 'samr'), 'manage_options', 'sidebars', array($this, 'add_submenu_page_cb')); 
	}
	...
	function add_submenu_page_cb( $html = null ) {
		...
		$html .= '<div id="sidebars-wrapper">';
			$html .= '<h1>'.__('Sidebars', 'samr').'</h1>';
			$html .= '<form method="post" id="add-sidebar" action="">';
				$html .= '<input type="text" id="add-sidebar-field" placeholder="'.__('Enter new sidebar name','samr').'">';
				$html .= '<input type="submit" id="add-sidebar-button" value="'.__('Add Sidebar','samr').'">';
			$html .= '</form>';
			$html .= '<div id="messages"></div>';
			$html .= '<h3>'.__('Sidebar List','samr').'</h3>';
			$html .= '<ul id="sidebar-list">';
			
			if ( $sidebars ) {
			
				foreach ($sidebars as $id => $name) {
					$html .= '<li>'.$name.' <code>[dynamic-sidebar id="'.$id.'"]</code><a href="#" data-sidebar-id="'.$id.'">'.__('Remove','samr').'</a></li>';
				}
				
			} else {
				$html .= '<li id="no-sidebar">'.__('You currently have no sidebars created. <br>Use the form above to create your first sidebar.','samr').'</li>';
			}
	...
	public function meta_box() {
		$screens = array( 'post', 'page' );
			...
			foreach ( $screens as $screen ) {
				add_meta_box('sidebar_metabox', __( 'Sidebar', 'samr' ), array($this, 'meta_box_callback'), $screen, 'side');
			}
	...
	public function meta_box_callback( $post ) {
		...
		echo '<p><strong><label>'.__('Choose Sidebar:','samr').'</label></strong></p>';
		echo '<select name="sidebar" id="sidebar">';
		echo '<option>'.__('Choose Sidebar','samr').'</option>';
		foreach ( $wp_registered_sidebars as $value ) {
			echo '<option value="'.$value['id'].'" '.selected($default_sidebar, $value['id']).'>'.$value['name'].'</option>';
		}
		echo '</select>';
		echo '<p>'.__('Choose a custom sidebar for this page','samr').'</p>';
	}
